...
// Validation schema — applied to every request body before processing.
// Accepts both camelCase and snake_case keys via preprocessing below.
const sendEmailSchema = z.object({
  templateName: z
    .string()
    .min(1, 'templateName is required')
    .max(100, 'templateName must be 100 characters or fewer')
    .regex(/^[a-zA-Z0-9_-]+$/, 'templateName may only contain letters, numbers, underscores or hyphens'),
  recipientEmail: z
    .string()
    .trim()
    .email('recipientEmail must be a valid email address')
    .max(254, 'recipientEmail must be 254 characters or fewer')
    .optional(),
  idempotencyKey: z
    .string()
    .min(1)
    .max(255)
    .regex(/^[a-zA-Z0-9_:-]+$/, 'idempotencyKey may only contain letters, numbers, underscores, colons or hyphens')
    .optional(),
      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          const normalized = {
            ...
            templateData: raw?.templateData ?? raw?.template_data,
          }
          const parsed = sendEmailSchema.safeParse(normalized)
          if (!parsed.success) {
            return Response.json(
              {
          ...
          idempotencyKey = parsed.data.idempotencyKey ?? messageId
          if (parsed.data.templateData) {
